...
class rectangle {
  ...
  checkCollision(
    direction //check for collision of this rect with wall
  ) {
    if (this.x + this.w >= canvas.width && this.dx > 0)
      //moving to right
      this.dx = 0;
  ...
  static checkRectCircCollision(
    b,
    ball //check for collison of a rect and a circle
  ) {
    if (
      !(
        b.x + b.w < ball.x - ball.radius ||
        b.x > ball.x + ball.radius ||
        b.y + b.h < ball.y - ball.radius ||
        ball.y + ball.radius < b.y
      ) &&
      b.display
    ) {
      //actually here ball is treated as square
      b.display = false;
...
class circle {
  ...
  checkCollision() {
    //check for collision of this ball with all other physical objects
    for (
      let i = 0;
      i < row;
      i++ //for all the bricks
    ) {
      for (let j = 0; j < column; j++)
...
function init() {
  ...
  ctx.fillStyle = "teal";
  bricks = new Array(6);
  for (let i = 0; i < row; i++) bricks[i] = new Array(column);
  for (let i = 0; i < row; i++) {
    for (let j = 0; j < column; j++)
...
function updateCanvas() {
  //the main runner which renders canvas continously in real time
...
function updateBar(e) {
  //updating the catcher bar below the ball
  let direction = undefined;
  const parentBtn = e.target.closest(".button");
  if ((e.type === "mousedown" || e.type === "mouseup") && !parentBtn) return;
  if (
    (e.type === "mousedown" && parentBtn === leftBtn) ||
  ...
  if (
    (e.type === "mousedown" && parentBtn === rightBtn) ||
  ...
  if (
    (e.type === "mouseup" && parentBtn === leftBtn) ||
    (e.type === "keyup" && e.key === "ArrowLeft")
  )
    bar.dx = 0;
  if (
    (e.type === "mouseup" && parentBtn === rightBtn) ||
    (e.type === "keyup" && e.key === "ArrowRight")
  )
    bar.dx = 0;
  if (direction && !gameStarted) ball.circleLaunch(direction);
}

function reset(displayMessage) {
  //resets canvas based on given conditions
  lives.value--;
  if (lives.value === 0) {
    //game over or won
    ...
    score.value = 0;
    for (let i = 0; i < row; i++) {
      for (let j = 0; j < column; j++) bricks[i][j].display = true;
    }
  }
  gameStarted = false;
  ball.dx = 0;
